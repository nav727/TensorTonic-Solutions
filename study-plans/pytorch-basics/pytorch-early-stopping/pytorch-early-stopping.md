...
$$
\mathcal{L}_v^* = \min_{i < e} \mathcal{L}_v^{(i)}
$$

Before the first epoch, the best loss is positive infinity. At each epoch $e$:
...
- **Using the stopped-epoch model**: The stopped epoch is $p$ epochs after the best epoch, so the model at the stopped epoch has degraded from the peak. Restoring the best checkpoint is an optional extension, not part of this problem
